...
analyze the codebase, and adopt [Missing AI Patterns](https://github.com/rajanbor/map/tree/main/library).
...
| `map optimize [path]` | Estimate context tokens, detect repeated blocks, save a report, or enforce a budget with `--check`. |
...
.map/
  ...
  tools/              # tool policies and token budgets
...
src/
  ...
  optimization/            token-budget analysis and duplicate-context detection
...
pnpm typecheck
pnpm test                # full Vitest suite used by CI
pnpm test:coverage       # local coverage report and thresholds
